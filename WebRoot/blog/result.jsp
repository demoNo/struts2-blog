...
<html>
  ...
  <body>
	...
	<div class="span-15" align="left">
	    <c:if test="${empty requestScope.sessay}">
          <p><span>对不起，没有匹配的搜索结果.</span></p>
        </c:if> 
        <c:if test="${!empty requestScope.sessay}">
		<c:forEach items="${requestScope.sessay}" var="es">
		     <div id="artical" class="box">
    			 <div id="es_title"><p><h2><a onfocus="this.blur()" title='${ es.title  }' href="essay/${ es.date }/${ es.essayid }"> ${ es.title  } </a>
    			 <span style="float:right"><a id="com" title="${ es.comments }条评论" href="essay/${ es.date }/${ es.essayid }">${ es.comments } </a></span></h2>
    			 </p></div>
    	 ...
    	 <s:url id="url_first" value="page/resfir"></s:url>    	 
         <s:url id="url_pre" value="pres.action">     
           <s:param name="pageNow" value="page.pageNow-1"></s:param>     
         </s:url>        
         <s:url id="url_next" value="nexts.action">     
            <s:param name="pageNow" value="page.pageNow+1"></s:param>     
         </s:url>          
         <s:url id="url_last" value="page/reslas"></s:url>  
         <s:if test="page.pageNow != 1">
         <s:a cssClass="page" href="%{url_first}" >首页</s:a>  
         <s:a cssClass="page" href="%{url_pre}" ><上一页</s:a> 
         </s:if>
         <s:if test="page.pageNow != page.lastPage && page.lastPage != 0"> 
    ...
    <div class="span-7 ">分类查看<hr>
      <div id="tag">
        <c:forEach items="${sessionScope.detail}" var="ess">
			<span><a title='${ ess.classification  }'
